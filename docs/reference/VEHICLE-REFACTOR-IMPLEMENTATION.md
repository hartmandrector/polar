...
**Key Constraint:** Maintain backward compatibility with existing tests (254+ tests, 0 errors required).

---

## Quick Reference Audit Checklist

> **Status:** All TODO(ref-audit) comments resolved (2026-02-20). This checklist is retained for future reference.

- **Mass calculations:** use `getVehicleMassReference(vehicle, polar)` → returns `pilotHeight_m` (1.875) for wingsuits, `referenceLength_m` for canopies
- **Aero calculations:** use `polar.referenceLength` → 1.93 for wingsuits (`A5_REF_LENGTH`), 1.875 for canopies (`IBEX_REF_LENGTH`)
- **Mixed normalization:** CG uses mass reference, positions use aero reference. For wingsuits: ~2.9% lever-arm discrepancy — intentional per Phase B
- **GLB/visual scaling:** render scaling reads from `ModelGeometry` registry (`glbToMeters`, `referenceHeight`), does not leak into physics
...
- [x] **Phase A:** ✅ `referenceLength` on every polar, bare `1.875` centralized (254/254 tests)
- [x] **Phase B:** ✅ Wingsuit ref = 1.93m (`A5_REF_LENGTH`); moment arms +2.9%, inertia +5.9%
- [x] **Phase C:** ✅ Vehicle registry, `getVehicleMassReference()`, debug panel, verification readouts, all TODO(ref-audit) resolved
...
- [x] **Documentation:** USER-MY-DATA.md complete; scenario walkthroughs updated
- [x] **Tests:** 254/254 passing; coverage includes all scenarios
...
1. **Phase D** — UI scaling controls (canopy area slider, pilot height slider)
2. **Phase E** — User integration guide validation with real user
3. **Structural cleanup** — extract GLBMetadata helper in vehicle-registry.ts, consolidate imports
