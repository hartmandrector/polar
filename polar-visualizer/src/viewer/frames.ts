...
/**
 * Gravity vector expressed in body-frame Three.js coordinates.
 *
 * In NED inertial frame, gravity = (0, 0, g).
 * Rotate into body frame: g_body = R_EB · g_inertial = R_EB · [0,0,g]
 * Then map from NED body to Three.js body.
 *
 * We only need φ and θ (yaw doesn't affect gravity in body frame — it's symmetric).
 */
export function gravityInBodyThreeJS(
  phi_rad: number,
  theta_rad: number,
  g: number = 9.81
): THREE.Vector3 {
  // R_EB * [0,0,g]  gives third column of R_EB scaled by g.
  // R_EB row-major:
  //   row2 col2 = cp*ct
  //   row1 col2 = sp*ct
  //   row0 col2 = -st
  ...
  // gravity in body NED = R_EB * [0,0,g]
  const gx = -st * g
  const gy = sp * ct * g
  const gz = cp * ct * g
...
/**
 * Build the 4×4 rotation matrix that transforms vectors from body frame
 * to inertial frame in Three.js coordinates. Used both by model-loader
 * and vectors so they share exactly the same rotation.
 *
 * This just wraps bodyToInertialQuat but returns a Matrix4 for applyMatrix4().
 */
export function bodyToInertialMatrix4(
  phi_rad: number,
  theta_rad: number,
  psi_rad: number
): THREE.Matrix4 {
  const q = bodyToInertialQuat(phi_rad, theta_rad, psi_rad)
  const m = new THREE.Matrix4()
...
/**
 * Convert Euler attitude angles (deg) to an {alpha, beta}-equivalent body rotation
 * that gives the same pitch-and-yaw orientation the old code expected.
 *
 * This is useful for backward compatibility: when the user has NOT touched
 * the attitude sliders, phi=0, theta=alpha, psi=-beta reproduces the legacy
 * behaviour.
 */
export function attitudeFromAlphaBeta(
  alpha_deg: number,
  beta_deg: number
