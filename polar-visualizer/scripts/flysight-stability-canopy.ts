/**
 * flysight-stability-canopy.ts — Canopy stability mode extraction from raw FlySight GPS data.
 *
 * Same pipeline as flysight-stability.ts but configured for canopy flight:
 * - Uses ibexulContinuous polar (paraglider/ram-air canopy)
 * - Eigenvalue prediction from canopy trim conditions
 * - Canopy-appropriate reference area, chord, mass
 *
 * Canopy orientation is approximated from GPS velocity (heading from groundtrack,
 * pitch from glide angle, AOA from polar table match). For higher accuracy,
 * use CloudBASE canopy normal (CN) vector export with gps-beta-enhance.ts.
 *
 * Usage:
 *   npx tsx scripts/flysight-stability-canopy.ts <track-csv> --window start,end
 *
 * Examples:
 *   npx tsx scripts/flysight-stability-canopy.ts ../public/05-04-25/TRACK.CSV --window 109,145
 */
...
import { a5segmentsContinuous, ibexulContinuous } from '../src/polar/polar-data.ts'
...
function buildPolarEvaluator(airspeed = 12, rho = 1.225): PolarEvaluator {
...
function computeEigenvaluePrediction(airspeed: number): EigenPrediction | null {
  ...
  const trim = findTrim(airspeed, config, { alphaGuess_deg: 10, thetaGuess_deg: -30 })
...
<h1>🐻‍❄️ FlySight Canopy Stability Analysis</h1>
<div style="color:#90a4ae; margin-bottom:20px;">${filename} — Polar Claw / ibexul canopy model</div>
...
function main() {
  ...
  console.log(`\n🐻‍❄️ FlySight Canopy Stability Analysis`)
  ...
  const result = processGPSFile(text, {
    ...
    pilotMass: 77.5,
    sRef: ibexulContinuous.s,  // canopy reference area (~20.4 m²)
  })
  ...
  const htmlPath = path.join(outDir, `${baseName}-canopy-stability.html`)
